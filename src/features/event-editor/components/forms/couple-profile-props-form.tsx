...
export function CoupleProfilePropsForm({ props, onChange, eventId }: FormProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <ImageUploadField label="사진" value={str(groom, 'photoUrl')} onChange={(v) => setGroom({ photoUrl: v })} eventId={eventId} cropAspect={1} hint="정사각형 비율 1:1" />
      ...
      <ImageUploadField label="사진" value={str(bride, 'photoUrl')} onChange={(v) => setBride({ photoUrl: v })} eventId={eventId} cropAspect={1} hint="정사각형 비율 1:1" />
      <div className="grid grid-cols-2 gap-2">
